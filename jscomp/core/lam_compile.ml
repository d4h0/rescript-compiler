...
*)
let rec compile_external_field ~output_prefix
    (* Like [List.empty]*)
...
*)
and compile_external_field_apply ~output_prefix (appinfo : Lam.apply)
...
*)
and compile_recursive_let ~output_prefix ~all_bindings
    (cxt : Lam_compile_context.t) (id : Ident.t) (arg : Lam.t) :
  ...
  | Lprim { primitive = Pmakeblock (_, _, _); args }
    when args_either_function_or_const args ->
      ( compile_lambda ~output_prefix
          { cxt with continuation = Declare (Alias, id) }
          arg,
        [] )
...
and compile_recursive_lets_aux ~output_prefix cxt (id_args : Lam_scc.bindings) :
    Js_output.t =
  (* #1716 *)
  let output_code, ids =
    Ext_list.fold_right id_args (Js_output.dummy, [])
      (fun (ident, arg) (acc, ids) ->
        let code, declare_ids =
          compile_recursive_let ~output_prefix ~all_bindings:id_args cxt ident
            arg
        in
  ...
  | _ -> (
      ...
      | first :: rest ->
          ...
          Ext_list.fold_left rest acc (fun acc x ->
              Js_output.append_output acc
                (compile_recursive_lets_aux ~output_prefix cxt x)))
...
and compile_general_cases :
      ...
      default_case ->
      J.block =
 fun ~output_prefix (make_comment : _ -> string option)
          ...
          [
            S.if_ ?declaration:define
              ...
              (Js_output.output_as_block
                 (compile_lambda ~output_prefix cxt lam));
          ])
  | [ (id, lam) ], Default x | [ (id, lam); (_, x) ], Complete ->
      morph_declare_to_assign cxt (fun cxt define ->
          let else_block =
            Js_output.output_as_block (compile_lambda ~output_prefix cxt x)
          in
          let then_block =
            Js_output.output_as_block (compile_lambda ~output_prefix cxt lam)
          in
  ...
  let compile_whole (cxt : Lam_compile_context.t) =
    ...
    | { block; value = Some e } -> (
        ...
        if sw_consts_full && sw_consts = [] then
          ...
          compile_cases ~output_prefix cxt e sw_consts sw_num_default
            get_const_name
        else
          ...
          let dispatch e =
            S.if_ (E.is_type_number e)
              ...
              ~else_:
                (compile_cases ~output_prefix cxt (E.tag e) sw_blocks
                   sw_blocks_default get_block_name)
          in
...
and compile_stringswitch ~output_prefix l cases default
    (lambda_cxt : Lam_compile_context.t) =
...
*)
and compile_staticcatch ~output_prefix (lam : Lam.t)
    (lambda_cxt : Lam_compile_context.t) =
    ...
    when Lam_compile_context.no_static_raise_in_handler code_table ->
      ...
      Js_output.append_output (Js_output.make declares)
        (Js_output.append_output lbody
           (compile_lambda ~output_prefix lambda_cxt handler))
  | _ -> (
      ...
      | NeedValue _ ->
          ...
          Js_output.append_output
            ...
            (Js_output.append_output lbody
               (Js_output.make
                  (compile_cases ~output_prefix new_cxt exit_expr handlers
                     NonComplete (fun _ -> None))
                  ~value:(E.var v)))
...
and compile_sequand ~output_prefix (l : Lam.t) (r : Lam.t)
    (lambda_cxt : Lam_compile_context.t) =
...
and compile_sequor ~output_prefix (l : Lam.t) (r : Lam.t)
    (lambda_cxt : Lam_compile_context.t) =
  if Lam_compile_context.continuation_is_return lambda_cxt.continuation then
...
and compile_for ~output_prefix (id : J.for_ident) (start : Lam.t)
    (finish : Lam.t) (direction : Js_op.direction_flag) (body : Lam.t)
    (lambda_cxt : Lam_compile_context.t) =
  let new_cxt = { lambda_cxt with continuation = NeedValue Not_tail } in
  let block =
    match
      ( compile_lambda ~output_prefix new_cxt start,
        compile_lambda ~output_prefix new_cxt finish )
    with
...
and compile_assign ~output_prefix id (lambda : Lam.t)
    (lambda_cxt : Lam_compile_context.t) =
  let block =
    match lambda with
...
*)
and compile_trywith ~output_prefix lam id catch
    (lambda_cxt : Lam_compile_context.t) =
  let aux (with_context : Lam_compile_context.t)
    ...
    [
      S.try_
        (Js_output.output_as_block
           (compile_lambda ~output_prefix body_context lam))
        ~with_:
          ( id,
            Js_output.output_as_block
              (compile_lambda ~output_prefix with_context catch) );
    ]
  in
  match lambda_cxt.continuation with
  | Declare (kind, id) ->
...
*)
and compile_ifthenelse ~output_prefix (predicate : Lam.t) (t_branch : Lam.t)
...
and compile_apply ~output_prefix (appinfo : Lam.apply)
    (lambda_cxt : Lam_compile_context.t) =
  match appinfo with
  | {
   ap_func =
...
and compile_prim ~output_prefix (prim_info : Lam.prim_info)
  ...
  | { primitive = Psequor; args = [ l; r ] } ->
      compile_sequor ~output_prefix l r lambda_cxt
  ...
  | {
   ...
   args =
     [
      ...
      in
      let obj_output =
        compile_lambda ~output_prefix need_value_no_return_cxt obj
      in
      let arg_output =
        compile_lambda ~output_prefix need_value_no_return_cxt setter_val
      in
...
and compile_lambda ~output_prefix (lambda_cxt : Lam_compile_context.t)
  ...
  | Llet (let_kind, id, arg, body) ->
      ...
      let args_code =
        compile_lambda ~output_prefix
          ...
          arg
      in
      Js_output.append_output args_code
        (compile_lambda ~output_prefix lambda_cxt body)
  ...
  | Lswitch (switch_arg, sw) ->
      compile_switch switch_arg ~output_prefix sw lambda_cxt
  | Lstaticraise (i, largs) ->
      compile_staticraise ~output_prefix i largs lambda_cxt
  ...
  | Lfor (id, start, finish, direction, body) -> (
      ...
      | ( Upto,
          ...
          compile_for id ~output_prefix start new_finish Up body lambda_cxt
      | _ ->
          compile_for id ~output_prefix start finish
